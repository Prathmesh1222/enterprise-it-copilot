# Enterprise IT Copilot: Technical Architecture Report

This document outlines the advanced architectural patterns and technical requirements implemented within the **Enterprise IT Copilot**.
...
*   **Reranking:** We implemented a robust Hybrid RAG pipeline. It retrieves documents using an `EnsembleRetriever` (combining FAISS semantic search and BM25 exact keyword matching) and then passes those documents through a **Cross-Encoder Reranker** (`ms-marco-MiniLM-L-6-v2`). This mathematically scores and re-orders the chunks, dramatically increasing accuracy for specific error codes.
*   **Measure Precision & Recall:** The project includes an `evaluate_rag.py` script. This script runs an evaluation dataset through the retrieval logic to calculate mathematical **Precision** (relevance of retrieved docs) and **Recall** (ability to find all relevant docs).
...
1.  **Document Search:** The `search_sops` tool triggers the Hybrid RAG pipeline. We also implemented a `query_knowledge_graph` tool simulating an in-memory GraphRAG architecture to resolve multi-hop dependencies.
2.  **Ticket System Lookup:** The `lookup_historical_tickets` tool queries a local SQLite database containing historical ticket resolutions. Furthermore, an `mcp_query_jira` tool mocks a Model Context Protocol connection for external live ticketing systems.
3.  **Summarizer:** The `summarize_document` tool allows the agent to intentionally summarize dense IT policies or lengthy ticket histories when the user specifically requests a high-level overview.
...
---
*Built for modern Enterprise IT Infrastructure.*
